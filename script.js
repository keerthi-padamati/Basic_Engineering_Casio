...
switch(op)
{
case "sum": res = nums.reduce(function(a,b){ return a+b; },0);
      break;
case "mean": res = nums.reduce(function(a,b){ return a+b; },0)/nums.length;
      break;
case "median": let mid = Math.floor(nums.length/2); 
            if(nums.length%2!=0) res = nums[mid]; 
            else res = (nums[mid-1]+nums[mid])/2; 
            break;
